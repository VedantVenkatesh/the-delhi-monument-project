...
$(window).scroll(function() {
    let bgwidth = window.screen.width;
    let scrollPos =  $(window).scrollTop() - $(".bg-4").offset().top*2;
    let bgPos = 100 + ((scrollPos / bgwidth) * 100)/($(".bg-4").attr("speed"));
    $(".bg-4").css("background-position", bgPos + "% 50%");
});

$(window).scroll(function() {
    let bgwidth = window.screen.width*1.2;
    let scrollPos = $(this).scrollTop();
    let bgPos = ((scrollPos / bgwidth) * 100)/($(".bg-3").attr("speed"));
    $(".bg-3").css("background-position", bgPos + "% 50%");
});
